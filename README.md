...
Create a GitHub repo (public or private — Pages works either way on a paid plan; public repos get Pages free).
Push these files to the repo, preserving the folder structure:
```
   bom_screener.html
   classify.py
   refresh_lists.py
   ...
   ```
Enable GitHub Pages: repo Settings → Pages → Source: "Deploy from a branch" → branch `main`, folder `/ (root)` → Save.
Enable Actions (usually on by default for a new repo): Settings → Actions → General → allow workflows to run.
Check the workflow has write permission: Settings → Actions → General → Workflow permissions → "Read and write permissions" (needed so it can commit the refreshed data back to the repo).
Done. Share the Pages URL with your team, e.g.:
```
